...
export class WebPageTestClient {
  ...
  constructor(server: string, key: string, interval: number) {
    ...
    this.timerId = global.setInterval(this.displayStatus.bind(this), interval);
  }
  ...
  async submitTest(
    ...
    return new Promise((resolve, reject) => {
      const id = `##${script.name}##${device.id}##${connectivity}`;
      const now = Date.now();
      ...
      log.info(
        `Submitting WebpageTest for [${script.name}] on [${device.id}] with ` +
          `[${connectivity}] for [${options.runs}] total runs ...`
      );
      this.instance.runTest(script.script, options, (error, result) => {
        if (error) {
          this.registry.get(id).status = TestStatus.Failed;
          log.warn(
            `WebpageTest for [${script.name}] on [${device.id}] with ` +
          ...
          log.info(
            `Received WebpageTest result [${result.data.id}] for ` +
              `[${script.name}] on [${device.id}] with [${connectivity}].`
          );
          resolve(result);
        }
      });
...
function constructPendingTestsString(pendings: TestRecord[]): string {
  ...
  if (pendings.length) {
    ...
    for (let i = 1; i <= pendings.length; i++) {
      ...
      str += `>> [${i}/${pendings.length}]: {name: ${p.name}, `;
